#include "testing.hpp"
void model_SURF_test (vector<Mat> images, vector<int> label, vector<int>& actual_label, vector<int>& predicted_label)
{
	...
	int new_match = 0;
	int old_match = 0;
	vector<KeyPoint> keypoints_test, keypoints_database;
  Mat descriptors_test, descriptors_database;
	...
	while (q < images.size())
	{
		...
		//checks the model
		surf->detectAndCompute(image_test, image_test, keypoints_test,  descriptors_test);
		for (int i = 0; i < images_1D_train.size(); i++)
		{
			FlannBasedMatcher matcher;
  		std::vector< DMatch > matches;
			surf->detectAndCompute(images_1D_train[i], images_1D_train[i], keypoints_database,  descriptors_database);
			matcher.match( descriptor_test, descriptor_database, matches );
			double max_dist = 0; double min_dist = 100;
			old_match = new_match;
			for (int x = 0; x < descriptor_test.rows; x++)
			{
			...
			for( int y = 0; y < descriptors_test.rows; y++ )
  		{ if( matches[y].distance < 2*min_dist )
    		{
